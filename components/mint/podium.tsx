...
/** Design 6A trial variant of components/podium.tsx, restyled for the light "mint" palette. */
      ...
      {order.map((rank) => {
        ...
        return (
          <div key={rank} className="flex flex-col items-center gap-2">
            {rank === 1 ? (
              <div className="rounded-full bg-mint-lime px-2 py-0.5 font-mint text-xs font-bold text-mint-lime-ink">KAMPIOEN</div>
            ) : null}
            <div className={`text-center text-sm leading-tight text-mint-ink ${rank === 1 ? "font-bold" : "font-semibold"}`}>
              {entry.name}
            </div>
            <div className={`flex w-full flex-col items-center justify-center gap-1 rounded-t-[24px] ${HEIGHTS[rank]} ${COLORS[rank]}`}>
