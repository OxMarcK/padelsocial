...
export interface CourtCardProps {
  ...
  highlight?: boolean;
  size?: "sm" | "lg";
  /** No match is tracked on this court this round — render an empty "Vrij te spelen" court instead of scores. */
  freePlay?: boolean;
}
...
export function CourtCard({ courtNumber, eyebrow, teamA, teamB, highlight, size = "sm", freePlay }: CourtCardProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="font-display text-sm font-bold uppercase tracking-wider text-net-grey">
        {freePlay ? "Vrij te spelen" : eyebrow}
      </div>
      <div
        className={`relative ${height} overflow-hidden rounded-2xl ${freePlay ? "bg-court-night" : "bg-glass-blue"} shadow-[inset_0_0_0_3px_rgba(245,247,250,.55)] ${
          highlight ? "ring-2 ring-lime-serve" : ""
        }`}
      >
        <div className={`absolute inset-y-0 left-1/2 w-[3px] -translate-x-1/2 bg-flood-white/90 ${freePlay ? "opacity-30" : ""}`} />
        <div className={`absolute inset-y-0 left-[22%] w-[2px] bg-flood-white/40 ${freePlay ? "opacity-30" : ""}`} />
        <div className={`absolute inset-y-0 left-[78%] w-[2px] bg-flood-white/40 ${freePlay ? "opacity-30" : ""}`} />
        <div className={`absolute left-0 top-1/2 h-[2px] w-[22%] bg-flood-white/40 ${freePlay ? "opacity-30" : ""}`} />
        <div className={`absolute right-0 top-1/2 h-[2px] w-[22%] bg-flood-white/40 ${freePlay ? "opacity-30" : ""}`} />
