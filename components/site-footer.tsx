 ...
 * the homepage's own mobile footer layout. `noTopMargin` drops the built-in
 * spacing above the footer — for callers that already sit directly below
 * their own last section (e.g. the admin shell's email/uitloggen bar). */
export function SiteFooter({ noTopMargin = false }: { noTopMargin?: boolean } = {}) {
  return (
    <footer className={`bg-[#0E2318] py-10 ${noTopMargin ? "" : "mt-14 sm:mt-[88px]"}`}>
